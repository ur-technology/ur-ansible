...
or environment variables (DO_API_TOKEN)\n''')
    ...
    def build_inventory(self):
	    ...
	    # Some nifty additions to the inventory by Jorge Vazquez
            if droplet['name'].startswith('miner'):
		self.push(self.inventory,"miner", dest)

	    if droplet['name'].startswith('boot'):
                   self.push(self.inventory,"bootnode", dest)

	    if droplet['name'].startswith('queue'):
                   self.push(self.inventory,"queue", dest)

	    if droplet['name'].startswith('relay'):
                   self.push(self.inventory,"transrelay", dest)

	    if droplet['name'].startswith('explorer'):
                   self.push(self.inventory,"explorer", dest)
