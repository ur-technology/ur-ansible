...
doctl compute floating-ip-action assign $DO_BOOTNODE_IP1 $DO_BOOTNODE_ID1  --wait --no-header
echo "Done."
echo "Now assigning floating IP to "$UR_BOOTNODE2_NAME" with ID "$DO_BOOTNODE_ID2"..."
doctl compute floating-ip-action assign $DO_BOOTNODE_IP2 $DO_BOOTNODE_ID2 --wait --no-header
echo "Done."
